...
pdf("Insect abundance vs device types.pdf")
draw(w2a,constant = coef(w2a)[1],select=3:4,fun=exp,grouped_by=T)+
  labs(
    title="",subtitle="",
    x = "Day (since start of observation period)",
    y = "Insect abundance",
    colour="Device type",
    fill="Device type",
    caption=NULL # get rid of "Basis: CRS (shrink)" caption in lower right corner of plot
    )+
  theme_classic(base_size = 12) +  # clean base, no grid
  theme(
    panel.border = element_rect(colour = "black", fill = NA, linewidth = 0.8), 
    axis.ticks.length = unit(-0.15, "cm"),   # short ticks, negative = inward
    axis.ticks = element_line(linewidth = 0.5), # thin ticks
    axis.text = element_text(size = 14, margin = margin(t = 4, r = 4, b = 4, l = 4)), # tick labels
    axis.title = element_text(size = 14, face = "bold"), # axis labels
    
    # Move tick labels
    axis.text.x = element_text(margin = margin(t = 6)), # bottom margin for x labels
    axis.text.y = element_text(margin = margin(r = 6))  # right margin for y labels
