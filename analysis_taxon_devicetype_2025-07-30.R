...
insecta_data <- insecta_data %>%
  group_by(DateTime, Day, Week, Device, Device_type, Site, Ambient, Class, Order, Suborder, Infraorder, Superfamily, Family, Subfamily, Genus, Species, AIRTEMP, WINDSPEED, Rain, RAD, GROUNDTEMP, RH, AIRP) %>%
...
m3 <- multinom(Order~ bs(Day, 3)*Device_type, data = insecta_data, maxit = 1000)
#m3 <- multinom(Order~ bs(DateTime, 3)*Device_type, data = insecta_data, maxit = 1000)


##################################################################################
# multinomial Figure (model-based):

#insecta_data$Order.ord=
#ordered(insecta_data$Order,levels=c("Dermaptera", "Hymenoptera", "foo", "Diptera", "Coleoptera", "Hemiptera", "Orthoptera", "Lepidoptera", "#N/C", "Pterygota", "Neuroptera"))

plot(allEffects(m3,xlevels=list(Day=1:20)),style="stacked",
     lattice=list(key.args=list(columns=3)),
     lines=list(col=order_colors)
     )
